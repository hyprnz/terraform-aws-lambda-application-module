...
This module supports the creation of AWS resources for a Lambda Application Architecture style service. This is a emergent design but has been used in a number of PROD solutions
...
*Internal* entrypoints are how functions in the Lambda Application can pass events to each other. All *internal* entrypoints will use EventBridge as the means to relay events and require some configuration. The configuration for this is still taking shape.

## Context

This is an opinionated module and as such requires a context for it to be an effective way to provision Lambda resources. It has been tested with the `Python 3.8 - 3.13`, `Nodejs 18 - 22` and recently with `Rust` functions & extensions Please do reach out of you are curious or interested in contributing to the work being done here.
...
| application_runtime | Lambda runtime for the application, only required for Zip package type. | `string` | `""` | no |
| application_version | The version of the zip package or container image tag to be deployed. | `string` | n/a | yes |
| application_package_type | The package Type of the Lambda Application, either `Zip` or `Image`. Defaults to `Zip` | `string` | `"Zip"` | no |
| artifact_bucket | Bucket that stores function artifacts. Includes layer dependencies. Required for zip-based deployment. | `string` | `""` | no |
| artifact_bucket_key | File name key of the artifact to load. Required for zip-based deployment. | `string` | `""` | no |
