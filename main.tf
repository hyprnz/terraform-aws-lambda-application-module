...
resource "aws_lambda_function" "lambda_application" {
  ...
  environment {
    variables = merge({ APP_NAME = var.application_name }, { PARAMETER_STORE_PATH = "${var.parameter_store_path}/${each.key}"}, local.datastore_env_vars, var.application_env_vars)
  }
