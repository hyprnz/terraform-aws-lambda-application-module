module "lambda_datastore" {
  source = "git::git@github.com:hyprnz/terraform-aws-data-storage-module?ref=2.0.2"
  providers = {
    aws = aws
  }
  ...
  tags          = merge({ "Lambda Application" = var.application_name }, var.datastore_tags, var.tags)
}
